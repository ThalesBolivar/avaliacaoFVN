-- ============================================================================
-- SEED: Perguntas da Ficha Fundamental (ANEXO III - LC 197/2019 - PCCV SAAE)
-- ----------------------------------------------------------------------------
-- Insere um MODELO de avaliação (RASCUNHO) e suas 17 perguntas com as
-- 4 alternativas (A/B/C/D) cada, na mesma ordem do documento.
--
-- Pré-requisitos:
--   * As 12 categorias já devem existir em `categorias_avaliacao`
--     para o município abaixo, com `ordem` de 1 a 12 (Assiduidade ... Trabalho em Equipe).
--   * As perguntas referenciam a categoria pela `ordem` (robusto, não depende de ID fixo).
--
-- Observações:
--   * Alternativas SEM pontuação (pontuacao = NULL), conforme solicitado.
--   * Rodar este script mais de uma vez cria um NOVO modelo a cada execução.
--   * Banco: MySQL.
--
-- Como rodar:
--   mysql -u USUARIO -p NOME_DO_BANCO < perguntas_ficha_fundamental.sql
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- 1) Modelo de avaliação (questionário)
-- ---------------------------------------------------------------------------
...
VALUES
    (@municipio_id,
     'Avaliação de Desempenho - Nível Fundamental',
     'Ficha Fundamental (ANEXO III) - LC 197/2019 - PCCV SAAE',
     1, 'RASCUNHO', 1, 1, 1, NOW());

SET @modelo_id = LAST_INSERT_ID();

-- Helper de leitura: pega o id da categoria pela ordem
-- (usado inline em cada pergunta abaixo)
...
SET @p := LAST_INSERT_ID();
INSERT INTO opcoes_pergunta_avaliacao (pergunta_avaliacao_id, letra_opcao, texto_opcao, pontuacao, criado_em) VALUES
(@p,'A','O servidor desempenha suas atividades com desenvoltura e de forma eficaz, demonstrando habilidade e conhecimento do que faz.',NULL,NOW()),
(@p,'B','O servidor desempenha as atividades rotineiras de forma satisfatória, com qualidade suficiente. O servidor tem dificuldades de executar serviços mais complexos ou que não sejam rotineiros.',NULL,NOW()),
(@p,'C','O servidor executa os trabalhos frequentemente com qualidade abaixo do esperado, realizando serviços rotineiros de forma muitas vezes incorreta.',NULL,NOW()),
(@p,'D','O servidor executa os trabalhos de forma inadequada, não realiza serviços rotineiros de forma satisfatória.',NULL,NOW());
...
VALUES
    (@modelo_id,
     ...
     'Considera a capacidade de aproveitamento dos recursos e racionalização dos serviços.',
     'MULTIPLA_ESCOLHA', 1.00, 1, 1, NOW());
SET @p := LAST_INSERT_ID();
INSERT INTO opcoes_pergunta_avaliacao (pergunta_avaliacao_id, letra_opcao, texto_opcao, pontuacao, criado_em) VALUES
(@p,'A','Sempre utiliza os materiais de trabalho de forma adequada, sem desperdiçá-los e buscando diminuir o consumo. Sempre apresenta ideias para simplificar, agilizar ou otimizar a rotina de trabalho.',NULL,NOW()),
(@p,'B','Utiliza constantemente os materiais de trabalho de forma adequada, buscando não desperdiçá-los. Frequentemente apresenta ideias para simplificar, agilizar ou otimizar a rotina de trabalho.',NULL,NOW()),
(@p,'C','Raramente utiliza os materiais de trabalho de forma adequada, muitas vezes desperdiçando-os. Raramente apresenta ideias para simplificar, agilizar ou otimizar a rotina de trabalho.',NULL,NOW()),
(@p,'D','Não se preocupa em utilizar os materiais de trabalho de forma adequada, desperdiçando-os. Não apresenta ideias para simplificar, agilizar ou otimizar a rotina de trabalho.',NULL,NOW());

-- ---------------------------------------------------------------------------
-- PERGUNTA 6 — RESPONSABILIDADE (categoria ordem 5)
-- ---------------------------------------------------------------------------
INSERT INTO perguntas_avaliacao
    (modelo_avaliacao_id, categoria_id, criterio, numero_pergunta, texto_pergunta,
     tipo_resposta, peso, obrigatoria, ativa, criado_em)
...
SET @p := LAST_INSERT_ID();
INSERT INTO opcoes_pergunta_avaliacao (pergunta_avaliacao_id, letra_opcao, texto_opcao, pontuacao, criado_em) VALUES
(@p,'A','Assume total responsabilidade pela execução de seu trabalho, cumprindo eficazmente as tarefas nos prazos estabelecidos.',NULL,NOW()),
(@p,'B','Aceita a responsabilidade pela execução de seu trabalho, cumprindo suas obrigações e no prazo solicitado com razoável frequência.',NULL,NOW()),
...
(@p,'D','Não assume a responsabilidade pela execução de seus trabalhos, desrespeitando o cumprimento de obrigações.',NULL,NOW());

-- ---------------------------------------------------------------------------
-- PERGUNTA 7 — INICIATIVA, DINAMISMO E EFICIÊNCIA (categoria ordem 6)
-- ---------------------------------------------------------------------------
INSERT INTO perguntas_avaliacao
    (modelo_avaliacao_id, categoria_id, criterio, numero_pergunta, texto_pergunta,
     tipo_resposta, peso, obrigatoria, ativa, criado_em)
VALUES
    (@modelo_id,
     ...
     'INICIATIVA, DINAMISMO E EFICIÊNCIA', 7,
...
-- ---------------------------------------------------------------------------
-- PERGUNTA 9 — ÉTICA NO SERVIÇO PÚBLICO (categoria ordem 7)
-- ---------------------------------------------------------------------------
INSERT INTO perguntas_avaliacao
    (modelo_avaliacao_id, categoria_id, criterio, numero_pergunta, texto_pergunta,
     tipo_resposta, peso, obrigatoria, ativa, criado_em)
...
SET @p := LAST_INSERT_ID();
INSERT INTO opcoes_pergunta_avaliacao (pergunta_avaliacao_id, letra_opcao, texto_opcao, pontuacao, criado_em) VALUES
(@p,'A','Comporta-se com ética, sigilo e discrição. Não pratica discriminação aos colegas de trabalho, superiores e público em geral.',NULL,NOW()),
...
(@p,'C','Apresenta dificuldade e necessita de orientação quanto a ética, sigilo, discrição, bem como em não discriminar colegas, superiores e público em geral.',NULL,NOW()),
...
-- ---------------------------------------------------------------------------
-- PERGUNTA 10 — EQUILIBRIO EMOCIONAL (categoria ordem 8)
-- ---------------------------------------------------------------------------
INSERT INTO perguntas_avaliacao
    (modelo_avaliacao_id, categoria_id, criterio, numero_pergunta, texto_pergunta,
     tipo_resposta, peso, obrigatoria, ativa, criado_em)
...
-- ---------------------------------------------------------------------------
-- PERGUNTA 11 — DEDICAÇÃO (categoria ordem 9)
-- ---------------------------------------------------------------------------
INSERT INTO perguntas_avaliacao
    (modelo_avaliacao_id, categoria_id, criterio, numero_pergunta, texto_pergunta,
     tipo_resposta, peso, obrigatoria, ativa, criado_em)
...
-- ---------------------------------------------------------------------------
-- PERGUNTA 12 — SOCIABILIDADE E COOPERAÇÃO (categoria ordem 10)
-- ---------------------------------------------------------------------------
INSERT INTO perguntas_avaliacao
    (modelo_avaliacao_id, categoria_id, criterio, numero_pergunta, texto_pergunta,
     tipo_resposta, peso, obrigatoria, ativa, criado_em)
...
-- ---------------------------------------------------------------------------
-- PERGUNTA 13 — SOCIABILIDADE E COOPERAÇÃO (categoria ordem 10)
-- ---------------------------------------------------------------------------
INSERT INTO perguntas_avaliacao
    (modelo_avaliacao_id, categoria_id, criterio, numero_pergunta, texto_pergunta,
     tipo_resposta, peso, obrigatoria, ativa, criado_em)
...
-- ---------------------------------------------------------------------------
-- PERGUNTA 14 — APRESENTAÇÃO PESSOAL, CAPACIDADE DE ORGANIZAÇÃO E FACILIDADE DE EXPRESSÃO (categoria ordem 11)
-- ---------------------------------------------------------------------------
INSERT INTO perguntas_avaliacao
    (modelo_avaliacao_id, categoria_id, criterio, numero_pergunta, texto_pergunta,
     tipo_resposta, peso, obrigatoria, ativa, criado_em)
VALUES
    (@modelo_id,
     ...
     'APRESENTAÇÃO PESSOAL, CAPACIDADE DE ORGANIZAÇÃO E FACILIDADE DE EXPRESSÃO', 14,
...
-- ---------------------------------------------------------------------------
-- PERGUNTA 15 — APRESENTAÇÃO PESSOAL, CAPACIDADE DE ORGANIZAÇÃO E FACILIDADE DE EXPRESSÃO (categoria ordem 11)
-- ---------------------------------------------------------------------------
INSERT INTO perguntas_avaliacao
    (modelo_avaliacao_id, categoria_id, criterio, numero_pergunta, texto_pergunta,
     tipo_resposta, peso, obrigatoria, ativa, criado_em)
VALUES
    (@modelo_id,
     (SELECT id FROM categorias_avaliacao WHERE municipio_id=@municipio_id AND ordem=11 AND ativo=1 LIMIT 1),
     'APRESENTAÇÃO PESSOAL, CAPACIDADE DE ORGANIZAÇÃO E FACILIDADE DE EXPRESSÃO', 15,
...
-- ---------------------------------------------------------------------------
-- PERGUNTA 16 — APRESENTAÇÃO PESSOAL, CAPACIDADE DE ORGANIZAÇÃO E FACILIDADE DE EXPRESSÃO (categoria ordem 11)
-- ---------------------------------------------------------------------------
INSERT INTO perguntas_avaliacao
    (modelo_avaliacao_id, categoria_id, criterio, numero_pergunta, texto_pergunta,
     tipo_resposta, peso, obrigatoria, ativa, criado_em)
VALUES
    (@modelo_id,
     (SELECT id FROM categorias_avaliacao WHERE municipio_id=@municipio_id AND ordem=11 AND ativo=1 LIMIT 1),
     'APRESENTAÇÃO PESSOAL, CAPACIDADE DE ORGANIZAÇÃO E FACILIDADE DE EXPRESSÃO', 16,
...
-- ---------------------------------------------------------------------------
-- PERGUNTA 17 — TRABALHO EM EQUIPE (categoria ordem 12)
-- ---------------------------------------------------------------------------
INSERT INTO perguntas_avaliacao
    (modelo_avaliacao_id, categoria_id, criterio, numero_pergunta, texto_pergunta,
     tipo_resposta, peso, obrigatoria, ativa, criado_em)
...
--   SELECT numero_pergunta, criterio, texto_pergunta
--     FROM perguntas_avaliacao WHERE modelo_avaliacao_id = @modelo_id
--    ORDER BY numero_pergunta;
--
--   SELECT COUNT(*) AS total_perguntas FROM perguntas_avaliacao WHERE modelo_avaliacao_id = @modelo_id; -- esperado: 17
--   SELECT COUNT(*) AS total_opcoes
--     FROM opcoes_pergunta_avaliacao o
--     JOIN perguntas_avaliacao p ON p.id = o.pergunta_avaliacao_id
--    WHERE p.modelo_avaliacao_id = @modelo_id; -- esperado: 68
-- ============================================================================
